"""Initial schema — baseline import of schema_v2.sql

This is a BASELINE migration, not an autogenerated one: it wraps the
existing bootstrap schema (database/schema/schema_v2.sql, 54 tables,
Chief Database Architect revision) as raw SQL, because SQLAlchemy models
only exist for 4 of the 25 modules so far (see alembic/env.py docstring).

Every future migration should be autogenerated (`alembic revision
--autogenerate -m "..."`) once a module's models.py exists — do NOT hand
edit this file. When schema_v2.sql and this migration diverge (they must
not), this migration is the source of truth going forward; schema_v2.sql
becomes a historical reference only.
...
def downgrade() -> None:
    # A full DROP of every object created above, in reverse dependency
    # order. DROP SCHEMA CASCADE is the pragmatic choice for a single
    # baseline migration — never used again once module-level migrations
    # take over (each of those gets a real, narrow downgrade()).
